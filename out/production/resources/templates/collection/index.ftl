    ...
    <div class="layui-body" style="background: #f5f5f5;">
        ...
        <div class="layui-row">
            ...
            <div style="padding: 15px;" class="layui-col-xs5">
                <form  class="form-inline" role="form" action="/collection/index">
        ...
        <table class="layui-table" lay-skin="line">
            ...
            <tr>
                ...
                <th>作者名称</th>
                <th>收藏日期</th>
                <th class="togoodsids">商品id</th>
                <th class="cuetomterids">用户id</th>
            </tr>
            </thead>
            <tbody>
                        <#list cotionsval as c>
                        <tr>
                            ...
                            <td>${c.author!''}</td>
                            <td>${c.redate?string('yyyy-MM-dd')}</td>

                            <td >
                                <a href="/goods/update/${c.goodsID!''}" title="" class="togoodsid">
                                ${c.goodsID!''}
                            </a>
                            </td>
                            <td>
                                <a href="/customer/update/${c.customerID!''}" title="" class="cuetomterid">
                                ${c.customerID!''}
                            </a>
                            </td>
        ...
        <div class="layui-col-xs12">
            ...
            <#if pageinfo.hasPreviousPage>
                <li><a href="/collection/index?page=${pageinfo.prePage}">上一页</a></li>
            </#if>
            <#list pageinfo.navigatepageNums as p>
                <li class="<#if "${p}"="${pageinfo.pageNum}">active</#if>"><a href="/collection/index?page=${p}">${p}</a></li>
            </#list>
            <#if pageinfo.hasNextPage>
                <li><a href="/collection/index?page=${pageinfo.nextPage}">下一页</a></li>
            </#if>
            </ul>
        </div>
        <div style="margin-left: 20px" class="layui-col-xs7">
            <div class="alert alert-info" role="alert">管理员可对用户的收藏夹进行管理</div>
        </div>
    </div>
...
<!--Local Stuff-->
<script>
    <#--console.log(${cotionsval});-->
    $(function () {
        layui.use([ 'element', 'layer'], function(){
            var element = layui.element; //导航的hover效果、二级菜单等功能，需要依赖element模块
            var layer = layui.layer //引入layer

            $(".cuetomterid").mouseover(function() {
                layer.tips('点击查看会员信息。', '.cuetomterids', {
                    tips: [1, '#3595CC'],
                    time: 1500
                });
            });
            $(".togoodsid").mouseover(function() {
                layer.tips('点击查看商品详情。', '.togoodsids', {
                    tips: [1, '#3595CC'],
                    time: 1500
                });
            });
